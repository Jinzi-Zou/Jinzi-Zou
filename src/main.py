...
def test(model, predictor, feat, edge_feat, graph, full_edge_feat, full_graph, split_edge, evaluator, batch_size, args):
    ...
    valid_pred = (torch.cat([pos_valid_pred, neg_valid_pred]) > 0.5).cpu().numpy()
...
def main():
    ...
    sys.stdout = Log(filename=f'{args.dataset}_standard_{args.model}_{args.loss_func}|{time.time()}.log', stream=sys.stdout)
    print(args)
    seed(args.seed)
