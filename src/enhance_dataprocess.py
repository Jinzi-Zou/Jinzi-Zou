...
def main():
    ...
    parser.add_argument('--p-model', type=str, default='agdn', choices=['gcn', 'gat', 'sage', 'agdn', 'memagdn'])
    ...
    parser.add_argument('--p-loss', type=str, default='AUC')
    ...
    parser.add_argument('--auxiliary_size', type=int, default=4000)
    parser.add_argument('--addtrain', action='store_true')
    #鲁棒损失函数参数
    ...
    # seed(args.seed)
    # sys.stdout = Log(filename=f'{args.dataset}_enh7nce_q03|{time.time()}.log', stream=sys.stdout)
    save_path = f'./result/log/{args.dataset}/enhance/'
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    sys.stdout = Log(save_path, filename=f'p-{args.p_model}_l-{args.model}_lo-{args.loss_func}_q-{args.q}_m-{args.auxiliary_size}|{time.time()}.log', stream=sys.stdout)
    print(args)
    ...
    candidate_link = torch.load(f'./result/candidate/{args.dataset}/p-{args.p_model}_lo-{args.p_loss}_cl.pt')
    extra_index = candidate_link['candidate_index'].to(device)

    #534850
    extra_index = extra_index[:args.auxiliary_size]

    full_graph.add_edges(extra_index[:, 0], extra_index[:, 1]
                         )


    # full_graph.add_edges(extra_index[:, 0], extra_index[:, 1],
    #                      {'weight': torch.ones(extra_index.size(0))})

    if args.addtrain:
        split_edge['train']['edge'] = torch.cat([split_edge['train']['edge'], extra_index[::2].cpu()])
    # split_edge['train']['edge'] = torch.cat([split_edge['train']['edge'], extra_index.cpu()])
    # split_edge['train']['weight'] = torch.cat([split_edge['train']['weight'], torch.ones(530000 / 2)])
    loss_dict = {}
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            ...
            if args.loss_func =='RoL':
                ro_total_loss,auc_total_loss = train(model, predictor, feat, full_edge_feat, full_graph, split_edge, optimizer,
                             args.batch_size, args)
                total_loss = ro_total_loss + auc_total_loss
                save_roloss.append(ro_total_loss)
                save_aucloss.append(auc_total_loss)
                save_totalloss.append(total_loss)
            else:
            ...
            if epoch % args.eval_steps == 0:
                ...
                for key, result in results.items():
                    ...
                    if key not in best_val:
                        ...
                        best_test[key] = result[2]
                        if key == target_metrics[args.dataset]:
                            save_path = f'./result/new_model/enhance/{args.dataset}/'
                            if not os.path.exists(save_path):
                                os.makedirs(save_path)
                            save_model(os.path.join(save_path, f'p-{args.p_model}_l-{args.model}_lo-{args.loss_func}_q-{args.q}_m-{args.auxiliary_size}_r-{run+1}.pt'), epoch, optimizer, model,
                                       predictor,
                                       emb)
                ...
                if epoch % args.log_steps == 0:
                    ...
                    if args.loss_func =='RoL':
                        print(f'---ro_Loss: {ro_total_loss:.4f}---aucloss:{auc_total_loss:.4f}---total_loss:{total_loss}\nTrain time: {(t2-t1):.4f}---Test time: {(t3-t2):.4f}---')
                    else:
                        print(
                            f'---loss:{loss}\nTrain time: {(t2 - t1):.4f}---Test time: {(t3 - t2):.4f}---')
            if args.adjust_lr:
        ...
        if args.loss_func == 'RoL':
            loss_dict[f'run{run+1}'] = {'ro_loss': save_roloss, 'auc_loss': save_aucloss,'total_loss':save_totalloss,'best_result':best_result}
        else:
            loss_dict[f'run{run+1}'] = {'loss':save_loss}
        for key in loggers.keys():
            print(key)
            loggers[key].print_statistics(run)
    save_path = f'./result/loss/enhance/{args.dataset}/'
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    pickle.dump(loss_dict, file=open(os.path.join(save_path,
                                                  f'p-{args.p_model}_l-{args.model}_lo-{args.loss_func}_q-{args.q}_m-{args.auxiliary_size}.pkl'), 'wb+'))
